...
class IncApp(Application):
    ...
    def main(self, ctl: Control, files: Sequence[str]):
        ...
        # creates new file: initplan.lp
        with open('initplan.lp', 'w') as f:
            # main program
            while ((conf.imax is None or step < conf.imax) and
                ...
                # filter and write the output in file
                ret = ctl.solve(on_model=lambda model: f.writelines(f"{atom}.\n" for atom in model.symbols(shown=True)))
                step += 1
